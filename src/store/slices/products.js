...
const productsSlice = createSlice({
    ...
    reducers: {
        ...
        productBooking: (state) =>{
            let index = state.products.findIndex(item => item.code === state.selected.code)
            if(index !== -1){
        ...
        returnProduct: (state) =>{
            let mainIndex = state.products.findIndex(item => item.code === state.selected.code)
            let index = state.bookings.findIndex(item => item.code === state.selected.code)
            if(index !== -1){
