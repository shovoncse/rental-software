...
const BookModal = ({ visible, setBookModal, setConfirmModal }) => {
    ...
    const handleBookOk = ({ product, date }) => {
        ...
        if (selected.minimum_rent_period <= days) {
            ...
            dispatch({ type: "products/getCalculatedPrice", payload: { days, date } })
                    ...
                    <Col span={10}>
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    {products.filter(item => item.availability && !item.needing_repair).map((data, index) => (
                                        <Option value={data.code} key={index}>{data.name}</Option>
                                    ))}
                                </Select>
                            </Form.Item>
                            <Form.Item name="date" label="Selete Date" className='font-semibold' rules={[
                                {
                                    required: true,
                    ...
                    <Col span={10}>
                        {selected ?
                            <List size="small" bordered className='bg-sky-100'>
                                <List.Item><span className='text-sm font-bold'>{selected.name}</span></List.Item>
                                <List.Item><span className='font-semibold'>Mileage:</span> {selected.mileage || '0'}</List.Item>
                                <List.Item><span className='font-semibold'>Price:</span> {selected.price}</List.Item>
                                <List.Item><span className='font-semibold'>Min Rent:</span> For <span className='font-bold'>{selected.minimum_rent_period}</span> Day(s)</List.Item>
                            </List> : null
                        }
                    </Col>
                </Row>
